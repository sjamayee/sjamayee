...
var Splitter = new Class({
    ...
    var w = 0, h = 0;
		if (parent) {
      	...
      	if (wrapper_initialSize.charAt(wrapper_initialSize.length-1) == '%') {
      	  var i = wrapper_initialSize.substr(0,wrapper_initialSize.length-1);
			    h = parent.getHeight() * i / 100;
			  } else {
  			  h = wrapper_initialSize.substr(0,wrapper_initialSize.length-2);
			  }
			} else {
      	if (wrapper_initialSize.charAt(wrapper_initialSize.length-1) == '%') {
      	  var i = wrapper_initialSize.substr(0,wrapper_initialSize.length-1);
  			  ...
  			  w = wrapper_initialSize.substr(0,wrapper_initialSize.length-2);
			  }
			}
		}
    ...
    if (wrapper_orientation && wrapper_orientation === 1) {
		...
		$extend(wrapper, {
		  ...
		  setSizep:this.setSizep,
		  getSizep:this.getSizep
		});
  ...
  addWidget: function(widget,parameters) {
      ...
      handle.addEvent('mousedown', function() {
    	  if (handleWidth === 0) { alert("handle/mousedown - handleWidth: 0"); return; }
    		...
    		var mouseup = function(e) {
    			...
    			//var p100 = orientation?parent.offsetHeight:parent.offsetWidth;
    			var sizes = parent.getSizes();
    			sizes[handle.num-1] += diff;
    			sizes[handle.num] -= diff;
    			parent.setSizes(sizes);
    			/*var sizep = [];
    			sizep.push(sizes[handle.num-1] / p100);
    			sizep.push(sizes[handle.num] / p100);
    			parent.setSizep(sizep);*/
    ...
    if (initialSize.charAt(initialSize.length-1) == '%') {
  	  var i = initialSize.substr(0,initialSize.length-1);
  		orientation?(wwHeight = parent.offsetHeight * i / 100, wwWidth = parent.offsetWidth)
  							 :(wwHeight = parent.offsetHeight, wwWidth = parent.offsetWidth * i / 100);
  	} else {
  		orientation?(wwHeight = initialSize.substr(0,initialSize.length-2), wwWidth = parent.offsetWidth)
  							 :(wwHeight = parent.offsetHeight, wwWidth = initialSize.substr(0,initialSize.length-2));
  	}
...
/*	
		var sizep = parent.getSizep();
		var p100 = orientation?h:w;
		var sizes = this.getSizes();
		sizes[0] = p100 * sizep[0] / 100 - 50;
		sizes[1] = p100 * sizep[1] / 100 + 500;
		this.setSizes(sizes);
  	for (var i = 0; i < parent.widgets.length; ++i) {
  	  //this.orientation?this.widgets[i].offsetHeight:this.widgets[i].offsetWidth);
  	  //this.orientation?this.widgets[i].setStyle('height', sizep[i]+'%'):this.widgets[i].setStyle('width', sizep[i]+'%');
  	  this.orientation?parent.widgets[i].setStyle('height', '10%'):parent.widgets[i].setStyle('width', '90%');
  	}
*/
  },
  ...
  setSizep: function(sizep) {
    this.p1 = this.p2 = 100;
    if (sizep) {
      if (sizep.length > 0) { this.p1 = sizep[0]; }
      if (sizep.length > 0) { this.p2 = sizep[1]; }
    }
  },
  getSizep: function() {
  	var result = [];
...
function resizePanes(left,right) {
	...
	if ($(left))	{ children = $(left).getElementsByTagName('div'); }
	//if ($(right)) { children = children.concat($(right).getElements('div')); }
	if ($(right)) { children = children.concat($(right).getElementsByTagName('div')); }
	children.each(function(child) {
		if (child.parentResized) child.parentResized();
	});
}
...
var AttributeListUIComponent = new Class({
  ...
  initializationComplete: function() {
    //Add splitter.
    this.hsplitter = new Splitter($(this.id+AttributeListUIComponent.BODY_ID), {'handleWidth':1,'handleColor':'red'}); //'lightgray'}); //'red'});
...
var BottomPane = new Class({
  ...
  initializeChildren: function() {
    ...
    this.vsplitter = new Splitter($("bottomPane"), {
      'handleWidth':5,
      ...
      'minimumSize':'50px',
      'maximumSize':'100%', //'897px', //(925-28)
      'initialSize':'100px', //'50%', //'400px', //'50%',
      'opaqueResize':1});
		this.vsplitter.addWidget($("b2"), {
      'handleWidth':5,
      'handleColor':'black', //'inherit',
      'handleBorder':'none', //'0px solid red',
      'orientation':1,
