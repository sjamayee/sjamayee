...
var Splitter = new Class({
    ...
    /*if (wrapper_orientation && wrapper_orientation === 1) {
		...
		$extend(wrapper,{addWidget:this.addWidget, resize:this.resize, parentResized:this.parentResized, setSizes:this.setSizes, getSizes:this.getSizes});
		...
		//window.addEvent('resize',wrapper.resize);
		window.document.addEvent('resize',function() {
			//var children = $ES('*');
  ...
  resize: function() {
    //if (this.id && this.id.substr(this.id.length - 9) == '_splitter') {
      //alert("RESIZE - ID: "+this.id);
      var children = this.getElements('div');
    	children.each(function(child) {
    		if (child.parentResized) child.parentResized();
    	});
  ...
  addWidget: function(widget,parameters) {
    ...
    var handleClass = (orientation && orientation === 1)?'vsplitbar':'hsplitbar';
  	...
  	var minimumSize = parameters['minimumSize']?parameters['minimumSize']:'10%'; //this.minimumSize;
    ...
    //if (handleWidth > 0) {
      ...
      handle.setAttribute("id",parent.id+"_"+widget.id+"_handle");
    	handle.setStyles(orientation?{'border-width':'1px 0','cursor':'ns-resize','height':handleWidth-2+'px','width':this.offsetWidth,'left':0, 'top':this.offsetTop}
    															:{'border-width':'0 1px','cursor':'ew-resize','width':handleWidth-2+'px','height':this.offsetHeight,'left':this.offsetLeft, 'top':0});  
      ...
      handle.addEvent('mousedown', function() {
    		...
    		if (orientation) {
    			...
    			//max = (parent.handleWidth !== 0)?parent.offsetHeight - parent.handleWidth:parent.offsetHeight; //parent.initialSize;
    			/*var offsetHeight = 0;
    			if (initialSize.charAt(initialSize.length-1) == '%') {
        	  var i = Number(initialSize.substr(0,initialSize.length-1));
  			    offsetHeight = parent.offsetHeight * i / 100;
  			    //offsetHeight = this.offsetHeight * i / 100;
  			  } else {
    			...
    			//max = (parent.handleWidth !== 0)?parent.offsetWidth - parent.handleWidth:parent.offsetWidth; //parent.initialSize;
    			/*var offsetWidth = 0;
    			if (initialSize.charAt(initialSize.length-1) == '%') {
        	  var i = Number(initialSize.substr(0,initialSize.length-1));
  			    offsetWidth = parent.offsetWidth * i / 100;
  			    //offsetWidth = this.offsetWidth * i / 100;
  			  } else {
    	    ...
    	    //min = (orientation?parent.handles[this.num - 1].offsetTop:parent.handles[this.num - 1].offsetLeft) + parent.handleWidth;
    		  if (parent.handleWidth !== 0) {
    		    ...
    		    //min = parent.initialSize;
      			if (initialSize.charAt(initialSize.length-1) == '%') {
          	  var i = Number(initialSize.substr(0,initialSize.length-1));
    			    min = (orientation?parent.offsetTop:parent.offsetLeft) * i / 100;
    			    //min = (orientation?this.offsetTop:this.offsetLeft) * i / 100;
    			  } else {
    		...
    		if (this.num < parent.handles.length - 1) {
    		  ...
    		  if (parent.handleWidth !== 0) {
    		    ...
    		    //max = parent.initialSize;
      			if (initialSize.charAt(initialSize.length-1) == '%') {
          	  var i = Number(initialSize.substr(0,initialSize.length-1));
    			    max = (orientation?parent.offsetTop:parent.offsetLeft) * i / 100;
    			    //max = (orientation?this.offsetTop:this.offsetLeft) * i / 100;
    			  } else {
    		...
    		var mouseup = function(e) {
          	  ...
          	  var m = Number(parent.minimumSize.substr(0,parent.minimumSize.length-1));
    			    parentMinimumSize = (orientation?parent.offsetTop:parent.offsetLeft) * m / 100;
    			  } else {
          	  ...
          	  var m = Number(parent.maximumSize.substr(0,parent.maximumSize.length-1));
    			    parentMaximumSize = (orientation?parent.offsetTop:parent.offsetLeft) * m / 100;
    			  } else {
    ...
    if (initialSize.charAt(initialSize.length-1) == '%') {
  	  var i = Number(initialSize.substr(0,initialSize.length-1));
  		orientation?(wwHeight = this.offsetHeight * i / 100, wwWidth = this.offsetWidth)
  						   :(wwHeight = this.offsetHeight, wwWidth = this.offsetWidth * i / 100);
  	} else {
  	  var i = Number(initialSize.substr(0,initialSize.length-2));
  		orientation?(wwHeight = i, wwWidth = this.offsetWidth)
  	...
  	var sizes = [];
  	var w = 0;
  	if (numWidgets) w += numWidgets * this.handleWidth; //handleWidth;
  	...
  	sizes.push((orientation?parent.offsetHeight:parent.offsetWidth) - w);
  	widgetWrapper.minimumSize = $pick(parameters.minimumSize,'0%');
    var widgetWrapperMinimumSize = 0;
  	if (widgetWrapper.minimumSize.charAt(widgetWrapper.minimumSize.length-1) == '%') {
  	  var i = Number(widgetWrapper.minimumSize.substr(0,widgetWrapper.minimumSize.length-1));
	    widgetWrapperMinimumSize = (orientation?this.offsetHeight:this.offsetWidth) * i / 100;
	  } else {
		  widgetWrapperMinimumSize = Number(widgetWrapper.minimumSize.substr(0,widgetWrapper.minimumSize.length-2));
	  }
  	this.minimumSize += widgetWrapperMinimumSize;
...
var AttributeListUIComponent = new Class({
  ...
  initialize: function(name,properties) {
    ...
    var html = '<div id="'+name+AttributeListUIComponent.HEADER_ID+'" class="'+AttributeListUIComponent.HEADER_CLASS_ID+'">'+headerValue+'</div>'+
               '<div id="'+name+AttributeListUIComponent.BODY_ID+'" style="position:relative;float:top;top:23px;height:100%;width:100%;">'+
  ...
  initializationComplete: function() {
    //Add splitter.
    this.hsplitter = new Splitter($(this.id+AttributeListUIComponent.BODY_ID), {'handleWidth':1,'handleColor':'red'}); //,'initialSize':'50%'}); //'lightgray'}); //'red'});
		this.hsplitter.addWidget($(this.id+AttributeListUIComponent.NAMES_ID), {'initialSize':'100%'}); //,{'handleColor':'lightgray'}); //, {'handleWidth':1});
		this.hsplitter.addWidget($(this.id+AttributeListUIComponent.VALUES_ID), {'initialSize':'100%'}); //,{'handleColor':'lightgray'}); //, {'handleWidth':1});
  },
...
var ObjectsList = new Class({
  ...
  initializationComplete: function() {
    ...
    this.hsplitter = new Splitter($(this.id), {'handleWidth':1,'handleColor':'yellow'});
		...
		this.hsplitter.addWidget($(this.id+ObjectsList.DESC_COLUMN_ID), {'handleWidth':5,'handleColor':'red','initialSize':'50%'});
  },
