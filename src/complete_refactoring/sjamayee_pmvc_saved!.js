...
var GridListSplitter = new Class({
  ...
  initializeChildren: function() {
    ...
    **this.hsplitter = new Splitter($(GridListSplitter.ID), {'handleWidth':4,'handleColor':'lightgray'});//,'handleBorder':'10px solid red'});
    ...
    this.hsplitter = new Splitter($(GridList.ID), {'handleWidth':4}); //,'handleColor':'lightgray'});//,'handleBorder':'10px solid red'});
		//this.hsplitter.addWidget($(GridListLeft.ID)); //, {'handleWidth':4,'handleColor':'lightgray'});//,'handleBorder':'10px solid red'});
		//this.hsplitter.addWidget($(GridListRight.ID)); //, {'handleWidth':4,'handleColor':'lightgray'});//,'handleBorder':'10px solid red'});
  }
});
...
/*childrenInitialized: function() {
  ...
  buildHtml: function(name) {
    ...
    this.hsplitter = new Splitter($(name+"X"), {'handleWidth':1});
		this.hsplitter.addWidget($(name+ObjectsList.REF_COLUMN_ID));
		this.hsplitter.addWidget($(name+ObjectsList.NAME_COLUMN_ID));
		this.hsplitter.addWidget($(name+ObjectsList.TYPE_COLUMN_ID));
		this.hsplitter.addWidget($(name+ObjectsList.DESC_COLUMN_ID));
    return result;    
  }
});
...
var RelationsGrid = new Class({
  ...
  getColumnHeaderId: function(col,name)  {
    var listName = this.id;
    if (name) { listName = name; } 
    var result = listName+RelationsGrid.COLUMN_ID+Utils.pad(col,Sjamayee.ID_PAD_SIZE)+'h';
    return result;
  },
  getColumnHeaderAnchorId: function(col,name)  {
    var listName = this.id;
    if (name) { listName = name; } 
    var result = listName+RelationsGrid.COLUMN_ID+Utils.pad(col,Sjamayee.ID_PAD_SIZE)+'ha';
    return result;
  },
  getColumnId: function(col,name)  {
    var listName = this.id;
  ...
  getCellAnchorId: function(row,col,name)  {
    var listName = this.id;
    if (name) { listName = name; } 
    var result = listName+RelationsGrid.COLUMN_ID+'r'+Utils.pad(row,Sjamayee.ID_PAD_SIZE)+'c'+Utils.pad(col,Sjamayee.ID_PAD_SIZE)+'a';
    return result;
  },
  buildHtml: function(name) {
    //Create splitter.
    ...
    /*result += '<div id="'+columnId+'" style="height:100%;width:'+width+'%;display:'+display+';" class="'+columnClass+'">'+
      ...
      result += '</div>';
      //Add column splitter.
  		this.hsplitter.addWidget($(columnId));
    }
    return result;
  },
  removeClass: function(id) {
...
var DetailSplitter = new Class({
  ...
  initializeChildren: function() {
    ...
    this.hsplitter = new Splitter($(Detail.ID), {'handleWidth':4}); //,'handleColor':'lightgray','minimumSize':0});//,'handleBorder':'10px solid lime'});
...
var RelationsGridMediator = new Class({
  ...
  isCurrentCellIdOnRoot: function() {
    ...
    var leftColumnId = this.gridUIC.id+RelationsGridLeft.COLUMN_ID;
  ...
  onKeydown: function(evt,subEvent) {
    ...
    switch (subEvent) {
      ...
      if (row > this.getBeginOfList(true)) {
        ...
        var cellId = this.gridUICLeft.getCellId(row,column);
        this.setCurrentCellId(cellId);
      }
      break;
      case SjamayeeFacade.DOWN:
...
**
      ...
      /*this.gridUICLeft.hsplitter = new Splitter(this.gridUICLeft);
  		...
  		this.gridUICLeft.hsplitter.addWidget($(this.gridUICLeft.getColumnId(1)));*/
      //this.gridUIC.hsplitter = new Splitter(this.gridUIC);
  		this.gridUIC.hsplitter.addWidget($(this.gridUICLeft.getColumnId(0)));
  		this.gridUIC.hsplitter.addWidget($(this.gridUICLeft.getColumnId(1)));
      //Set show/hide
      /*$(this.gridUICLeft.getColumnId(0)).setAttribute("style","display:block;");
      $(this.gridUICLeft.getColumnId(1)).setAttribute("style","display:block;");
      if (Position.COLUMNS_MAX() > 2) {
  ...
  fillGrid: function() {
    ...
    if (column) {
      ...
      for (var row = this.getBeginOfList(); row <= RelationsGridMediator.PAGE_SIZE_MAX; row++) {        
        ...
        var cellId = this.gridUIC.getCellId(row);
        if ($(cellId)) {
          //Remove Cell Classes.
          $(cellId).removeClass(GridCell.WHAT_USED_FOCUSED_CLASS_ID);
          $(cellId).removeClass(GridCell.WHAT_USED_SELECTED_CLASS_ID);
        ...
        if (row <= this.getEndOfList()) {        
          ...
          /*if (cell.isSelected()) {
            ...
            var cellAnchorId = this.gridUIC.getCellAnchorId(row); //RelationsGridRight.COLUMN_ID+'c0'+row+'a';
