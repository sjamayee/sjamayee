...
var PrepViewCommand = new Class({
  ...
  /*alert("PrepViewCommand - app: "+app.appName);
    ...
    this.facade.registerMediator(new DataObjectNTDMediator(app.detail.left.dataObjectNTD));
    this.facade.registerMediator(new DataObjectPropertiesMediator(app.detail.right.dataObjectProperties));
    ...
    this.facade.registerMediator(new ModelObjectNTDMediator(app.detail.left.modelObjectNTD));
    this.facade.registerMediator(new ModelObjectPropertiesMediator(app.detail.right.modelObjectProperties));
...
var Splitter = new Class({
	initialize: function(parent,parameters) {
      ...
      //h = h - (parseInt(parent.getStyle('border-top-height')) - parseInt(parent.getStyle('border-bottom-height')));			
		...
		wrapper.setStyles({'position':'relative', 'background-color':'transparent', 'width':w, 'height':h}); //, 'overflow':'hidden'}
...
*/
    this.vsplitter = new Splitter($(Sjamayee.FORM), {
      ...
      'opaqueResize':0});
		this.vsplitter.addWidget($(Header.ID), {
      'handleWidth':3, //0,
      'handleColor':'red',
      'handleBorder':'3px solid red',
      'orientation':1,
      ...
      'initialSize':26,
      'opaqueResize':0});
		this.vsplitter.addWidget($(GridList.ID), {
      'handleWidth':0, //3,
      ...
      'initialSize':400,
      'opaqueResize':0});
		this.vsplitter.addWidget($(ToolBar.ID), {
      'handleWidth':0,
      'handleColor':'red',
      ...
      'initialSize':28,
      'opaqueResize':0});
		this.vsplitter.addWidget($(Detail.ID), {
      'handleWidth':10,
      'handleColor':'green',
...
var AttributeListUIComponent = new Class({
  ...
  initializationComplete: function() {
    //Add splitter.
    this.hsplitter = new Splitter($(this.id), {'handleWidth':1,'handleColor':'black'}); //'red'});
		this.hsplitter.addWidget($(this.id+AttributeListUIComponent.NAMES_ID),{'handleColor':'red'}); //, {'handleWidth':1});
		this.hsplitter.addWidget($(this.id+AttributeListUIComponent.VALUES_ID),{'handleColor':'red'}); //, {'handleWidth':1});
  },
...
var RelationsGrid = new Class({
  ...
  initializationComplete: function() {
    ...
    for (var col = Position.COLUMN_FIRST(); col < Position.COLUMNS_MAX(); col++) {
		  this.hsplitter.addWidget($(this.getColumnId(col)));
		}
  },  
...
var Detail = new Class({
  ...
  initializationComplete: function() {
		if (!(($(DetailLeft.ID).getStyle('display') == 'none') &&
		      ($(DetailMiddle.ID).getStyle('display') == 'none') &&
		      ($(DetailRight.ID).getStyle('display') == 'none'))) {
      this.hsplitter = new Splitter($(this.id), {'handleWidth':1, 'handleColor':'lime'});
		  if ($(DetailLeft.ID).getStyle('display') != 'none') {
		    this.hsplitter.addWidget($(DetailLeft.ID),{'initialSize':'100'});
		  }
		  if ($(DetailMiddle.ID).getStyle('display') != 'none') {
		    this.hsplitter.addWidget($(DetailMiddle.ID),{'initialSize':'400'});
		  }
		  if ($(DetailRight.ID).getStyle('display') != 'none') {
		    this.hsplitter.addWidget($(DetailRight.ID),{'initialSize':'800'});
		  }
		}
  },  
...
var DetailLeft = new Class({
  Extends: SjamayeeUIComponent,
  initialize: function() {
    this.parent(DetailLeft.ID);
  },
});
DetailLeft.ID = "dleft";

//Class: DetailMiddle
var DetailMiddle = new Class({
  Extends: SjamayeeUIComponent,
  initialize: function() {
    this.parent(DetailMiddle.ID);
  },
});
DetailMiddle.ID = "dmiddle";

//Class: DetailRight
var DetailRight = new Class({
  Extends: SjamayeeUIComponent,
  initialize: function() {
    this.parent(DetailRight.ID);
  },
});
DetailRight.ID = "dright";
